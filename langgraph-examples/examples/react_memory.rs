...
impl LlmClient for MemoryMockLlm {
    async fn invoke(
        &self,
        messages: &[Message],
    ) -> Result<LlmResponse, langgraph::error::AgentError> {
...
impl ToolSource for MemoryToolSource {
    ...
    ) -> Result<ToolCallContent, langgraph::tool_source::ToolSourceError> {
        match name {
            "save_memory" => {
...
impl Node<MemoryReActState> for MemoryThinkNode {
    ...
    ) -> Result<(MemoryReActState, Next), langgraph::error::AgentError> {
