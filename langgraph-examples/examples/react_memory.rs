...
use langgraph::{
    Checkpointer, Embedder, JsonSerializer, LlmClient, LlmResponse, Message, Namespace,
    Node, RunnableConfig, StateGraph, Store, StoreError, ToolSource, Next, ToolSpec, ToolCallContent,
    ToolCall as ReActToolCall, ToolResult, START, END, LanceStore,
};
use langgraph::SqliteSaver;
...
impl LlmClient for MemoryMockLlm {
    async fn invoke(&self, messages: &[Message]) -> Result<LlmResponse, langgraph::error::AgentError> {
        ...
        let content = if let Some(Message::User(text)) = last_user_msg {
            ...
            } else if lower.contains("hobbies") || lower.contains("interests") || lower.contains("hobby") {
                extract_and_format_tool_call("retrieve_memory", "hobbies")
            } else if lower.contains("preferences") || lower.contains("what do i like") || lower.contains("what are my") {
                extract_and_format_tool_call("list_memories", "")
            } else if lower.contains("love") || lower.contains("like") || lower.contains("favorite") || lower.contains("prefer") {
                extract_and_format_tool_call("save_memory", text)
            } else {
                format!("Based on our conversation, here's my response to: {}", text)
            }
        } else {
            "I'm ready to help! Tell me your name or preferences, and I'll remember them.".to_string()
        };
...
fn extract_and_format_tool_call(tool_name: &str, text: &str) -> String {
    match tool_name {
        "save_memory" => {
            ...
            format!("tool_call:{{\"name\":\"{}\",\"arguments\":{}}}", tool_name, args)
        }
        "retrieve_memory" => {
        ...
        "list_memories" => {
            format!("tool_call:{{\"name\":\"{}\",\"arguments\":{{}}}}", tool_name)
        }
        _ => format!("tool_call:{{\"name\":\"{}\",\"arguments\":{{}}}}", tool_name),
    }
}
...
impl ToolSource for MemoryToolSource {
    async fn list_tools(&self) -> Result<Vec<ToolSpec>, langgraph::tool_source::ToolSourceError> {
        Ok(vec![
            ToolSpec {
    ...
    async fn call_tool(&self, name: &str, arguments: serde_json::Value) -> Result<ToolCallContent, langgraph::tool_source::ToolSourceError> {
        match name {
            "save_memory" => {
                let info = arguments["info"].as_str().unwrap_or("").to_string();
                let timestamp = chrono::Utc::now().to_rfc3339();
                let key = format!("memory_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0));
                let value = serde_json::json!({
                    "info": info,
                    "timestamp": timestamp
                });
                self.store.put(&self.namespace, &key, &value).await
                    .map_err(|e| langgraph::tool_source::ToolSourceError::Transport(e.to_string()))?;
                Ok(ToolCallContent {
            ...
            "retrieve_memory" => {
                ...
                let hits = self.store.search(&self.namespace, Some(key), Some(5)).await
                    .map_err(|e| langgraph::tool_source::ToolSourceError::Transport(e.to_string()))?;
                if hits.is_empty() {
                    Ok(ToolCallContent {
                    ...
                    let memories: Vec<String> = hits.iter()
            ...
            "list_memories" => {
                ...
                for key in keys {
                    if let Some(value) = self.store.get(&self.namespace, &key).await
                        .map_err(|e| langgraph::tool_source::ToolSourceError::Transport(e.to_string()))? {
            ...
            _ => Err(langgraph::tool_source::ToolSourceError::NotFound(format!("Unknown tool: {}", name))),
        }
    }
}
...
impl Node<MemoryReActState> for MemoryThinkNode {
    ...
    async fn run(&self, state: MemoryReActState) -> Result<(MemoryReActState, Next), langgraph::error::AgentError> {
    ...
    async fn run(&self, state: MemoryReActState) -> Result<(MemoryReActState, Next), langgraph::error::AgentError> {
        ...
        for tc in &state.tool_calls {
            ...
            let content = self
                ...
                .await
                .map_err(|e| langgraph::error::AgentError::ExecutionFailed(e.to_string()))?;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for memory in &sample_memories {
        let memory_key = format!("memory_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0));
        let memory_value = serde_json::json!({
            "info": memory,
            "timestamp": chrono::Utc::now().to_rfc3339(),
        });
        store
            .put(&namespace, &memory_key, &memory_value)
            .await?;
    }
    ...
    for (idx, query) in test_queries.iter().enumerate() {
        ...
        current_state.messages.push(Message::User(query.to_string()));

        match compiled.invoke(current_state.clone(), Some(config.clone())).await {
            Ok(new_state) => {
                let new_messages: Vec<_> = new_state.messages.iter()
                ...
                println!("[Short-term Memory] {} messages in session", new_state.messages.len());
    ...
    if let Some((cp, _)) = checkpoint {
        ...
        println!("✓ Final state has {} messages", cp.channel_values.messages.len());
    }
    ...
    println!("   Showing first 10 memories (total: {} stored):", all_keys.len());
