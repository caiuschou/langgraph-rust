...
///
/// ```no_run
/// use langgraph::tools::{BashTool, Tool};
/// use serde_json::json;
///
/// # #[tokio::main]
/// # async fn main() {
...
impl BashTool {
    ...
    ///
    /// ```
    /// use langgraph::tools::bash::BashTool;
    ///
    /// let tool = BashTool::new();
    /// ```
    pub fn new() -> Self {
...
impl Tool for BashTool {
    ...
    fn spec(&self) -> crate::tool_source::ToolSpec {
        crate::tool_source::ToolSpec {
            ...
            description: Some(
                "Run a shell command. Use this tool to execute system commands (e.g. list files, \
                 run scripts). On Unix uses sh -c; on Windows uses cmd /C. Returns combined stdout \
            ...
            input_schema: json!({
                ...
                "properties": {
                    "command": {
                        "type": "string",
                        "description": "The shell command to run (e.g. 'echo hello', 'ls -la')."
                    }
                },
                "required": ["command"]
            }),
