...
//!
//! ```rust,ignore
//! use langgraph::memory::{Store, Namespace};
//!
//! // Single operations
...
///
/// Aligns with LangGraph namespace tuple. Each element in the vector represents
/// one level in the hierarchy, allowing for nested categorization.
///
/// ## Example
///
/// ```rust
/// use langgraph::memory::Namespace;
///
...
///
/// Aligns with LangGraph `Item` class. Contains the stored value along with
...
///
/// Supports exact matches and comparison operators, aligning with LangGraph's filter syntax.
...
///
/// Aligns with LangGraph's Op types (GetOp, PutOp, SearchOp, ListNamespacesOp).
...
///
/// Aligns with LangGraph `BaseStore` class.
///
/// ## Core Operations
///
...
///
/// ```rust,ignore
/// use langgraph::memory::{Store, Namespace, SearchOptions};
///
