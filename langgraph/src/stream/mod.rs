...
//!
//! ```rust,ignore
//! use langgraph::stream::{StreamWriter, StreamMode};
//!
...
///
/// ```rust,ignore
/// use langgraph::stream::ToolStreamWriter;
/// use serde_json::json;
///
...
/// This aligns with Python LangGraph's checkpoint streaming format.
#[derive(Clone, Debug)]
...
///
/// ```rust,ignore
/// use langgraph::stream::StreamWriter;
///
/// async fn run_with_context(&self, state: S, ctx: &RunContext<S>) -> Result<(S, Next), AgentError> {
///     let writer = StreamWriter::from_context(ctx);
///     
