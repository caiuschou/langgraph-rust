...
fn llm_config_summary_entries_temperature_default_when_none() {
    ...
    assert_eq!(map.get("temperature").map(|v| v.as_str()), Some("(default)"));
...
fn memory_config_summary_section_name_and_entries_both() {
    ...
    assert_eq!(map.get("store").map(|v| v.as_str()), Some("in_memory_vector"));
}
...
fn tool_config_summary_section_name_and_entries_with_exa() {
    ...
    assert_eq!(map.get("exa_url").map(|v| v.as_str()), Some("https://mcp.exa.ai/mcp"));
}
...
fn embedding_config_summary_section_name_and_entries() {
    ...
    let entries = s.entries();
    let map: std::collections::HashMap<_, _> = entries.into_iter().collect();
    assert_eq!(map.get("model").map(|v| v.as_str()), Some("text-embedding-3-small"));
    assert_eq!(map.get("api_base").map(|v| v.as_str()), Some("https://api.openai.com/v1"));
}
