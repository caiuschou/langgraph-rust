...
//!
//! ```bash
//! cargo test -p langgraph exa_http -- --ignored
//! ```
...
#[ignore = "requires EXA_API_KEY and network; run with: cargo test -p langgraph exa_http -- --ignored"]
async fn exa_http_list_tools() {
    dotenv::dotenv().ok();
    let key = std::env::var("EXA_API_KEY").expect("EXA_API_KEY must be set in .env or env for exa_http tests");
    ...
    let known = ["web_search_exa", "get_code_context_exa", "company_research_exa"];
