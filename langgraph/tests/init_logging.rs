...
//! from the langgraph library (e.g. `tracing::debug!` in `openai.rs`) are printed
...
//!
//! ```bash
//! RUST_LOG=langgraph=debug cargo test -p langgraph -- --nocapture
//! RUST_LOG=debug cargo test -p langgraph openai_sse -- --nocapture
//! ```
